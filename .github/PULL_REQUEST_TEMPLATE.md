...
需要按需运行真实服务验证时，可额外提供一个 \`\`\`test fenced block。
When live service verification is useful, optionally provide one \`\`\`test fenced block.
只允许受信任白名单中的命令与受控管道；不会执行 shell。
Only trusted-whitelist commands and controlled pipelines are accepted; no shell is invoked.

\`\`\`test
javdb search ABC-123 --ndjson | javdb detail
\`\`\`
-->
