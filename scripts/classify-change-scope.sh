...
full_scope() { emit_scope false true true "$1" false; }
...
while IFS= read -r -d '' path; do
  ...
  case "$path" in
    Dockerfile|.dockerignore|go.mod|go.sum|LICENSE|cmd/*|internal/*|sdk/*|ci/*|tools/platformmatrix/*|scripts/build-release.sh) container_required=true ;;
  esac
done < "$changed"
