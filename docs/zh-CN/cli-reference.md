...
接受影片 `NUMBER` 的命令会去除首尾空白，并要求大小写不敏感的完整番号匹配。没有精确匹配或存在多个
精确匹配都会报错；命令不会选择模糊搜索结果的首项。状态变更命令只有在原始影片引用已确认是内部 ID 时才使用
`--id`。消费 `kind=movie` 管道信封时，非空 `id` 具有权威性；`--id` 只适用于原始引用或自身没有 ID 的信封。
