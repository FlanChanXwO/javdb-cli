...
Commands that accept a movie `NUMBER` trim surrounding whitespace and first try
a case-insensitive full-number match. If no exact match exists, one
unambiguous formatting-equivalent number (such as a separator difference) is
accepted. Multiple distinct movie IDs or fuzzy-only results are errors; commands
never select the first fuzzy search result. For a state-changing command, use
`--id` only when the raw movie reference is a confirmed internal ID. When
consuming a `kind=movie` pipeline envelope, a non-empty `id` is authoritative;
`--id` applies only to raw references or an envelope without its own ID.
