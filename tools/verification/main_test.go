...
func TestRunPipelineUsesRepositoryBinaryAndPipefail(t *testing.T) {
	...
	root := t.TempDir()
	binary := filepath.Join(root, "javdb")
	body := "#!/bin/sh\ncat >/dev/null\nprintf 'ok\\n'\n"
	if err := os.WriteFile(binary, []byte(body), 0o700); err != nil {
		t.Fatal(err)
	}
	command, err := verificationpolicy.ParseLine("echo ABC-123 | javdb detail")
	if err != nil {
		t.Fatal(err)
	}
	...
	command, err = verificationpolicy.ParseLine("echo x | grep y | javdb detail")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunPipelineOnlyAppliesExplicitPositiveTimeout(t *testing.T) {
	...
	command, err := verificationpolicy.ParseLine("javdb --version")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunPipelineAllowsDownstreamEarlyExit(t *testing.T) {
	...
	command, err := verificationpolicy.ParseLine("javdb rankings movies --period week | head -n 1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderShowsOnlyFailedCommandDetails(t *testing.T) {
	...
	results := map[string]platformResult{
		"linux/amd64": {
			...
			Commands: []commandResult{
				{Command: "javdb --version", Status: "passed"},
				{Command: "javdb detail ABC-123", Status: "failed", ExitCode: 1, DurationMS: 1200, Stages: []stageResult{{Command: "javdb detail ABC-123", Status: "failed", ExitCode: 1, Stderr: "boom"}}},
			},
		},
	}
	...
	if strings.Contains(body, "<code>javdb --version</code>") {
		t.Fatal("successful command should not be expanded")
	}
	for _, want := range []string{"javdb detail ABC-123", "Exit code", "1.2s", "boom", "View full GitHub Actions logs"} {
