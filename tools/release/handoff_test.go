...
func TestReleaseHandoffBindsIdentityAndChecksums(t *testing.T) {
	...
	appendPlatformArtifacts(t, dist, version,
		"javdb-cli_1.2.3_linux_amd64.tar.gz",
		"javdb-cli_1.2.3_darwin_arm64.tar.gz",
	)
	...
	if handoff.Repository != "FlanChanXwO/javdb-cli" {
	...
	for _, artifact := range append(append([]handoffArtifact{}, handoff.ProductionArtifacts...), handoff.ContainerArtifacts...) {
		var dir string
		if strings.HasPrefix(artifact.Name, "javdb-cli-") {
			dir = containers
		} else {
			dir = dist
		}
...
func TestVerifyHandoffArtifactsRejectsTamperedBytes(t *testing.T) {
	...
	dist := t.TempDir()
	containers := t.TempDir()
	appendPlatformArtifacts(t, dist, version, "javdb-cli_1.2.3_linux_amd64.tar.gz")
	appendPlatformArtifacts(t, containers, version, "javdb-cli-linux-amd64.tar")
	...
	if err := os.WriteFile(filepath.Join(dist, "javdb-cli_1.2.3_linux_amd64.tar.gz"), []byte("tampered\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := verifyHandoffArtifacts(handoff, dist, containers); err == nil {
...
func TestVerifyHandoffArtifactsRejectsIncompleteSet(t *testing.T) {
	...
	if err := os.Remove(filepath.Join(containers, "javdb-cli-linux-amd64.tar")); err != nil {
		t.Fatal(err)
	}
	if err := verifyHandoffArtifacts(handoff, dist, containers); err == nil {
