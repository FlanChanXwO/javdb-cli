...
func main() {
	if len(os.Args) < 2 {
		fatal(errors.New("expected verify-source, verify-published-release, verify-handoff, write-handoff, verify-handoff-set, verify-artifact-set, verify-container-set, or checksums"))
	}
	switch os.Args[1] {
	case "verify-source":
...
func verifyContainerSet(args []string) {
	...
	for _, entry := range entries {
		if !entry.IsDir() && strings.HasPrefix(entry.Name(), "javdb-cli-") && strings.HasSuffix(entry.Name(), ".tar") {
...
func expectedArtifacts(registryPath, version string) ([]string, error) {
	...
	for _, item := range cfg.Platforms {
		...
		names = append(names, fmt.Sprintf("javdb-cli_%s_%s_%s.%s", version, item.GOOS, item.GOARCH, extension))
	}
	if len(names) == 0 {
...
func expectedContainerArtifacts(registryPath string) ([]string, error) {
	...
	for _, item := range cfg.Platforms {
		...
		seen[key] = struct{}{}
		names = append(names, fmt.Sprintf("javdb-cli-%s-%s.tar", item.GOOS, item.GOARCH))
	}
	if len(names) == 0 {
