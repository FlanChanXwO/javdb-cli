...
func verifyPublishedReleaseCommand(args []string) {
	...
	if err := verifyPublishedRelease(client, *repository, *tag); err != nil {
		fatal(err)
	}
}
...
func verifyHandoffCommand(args []string) {
	...
	tag := set.String("tag", "", "optional immutable release tag for strict head binding")
	_ = set.Parse(args)
	...
	if strings.TrimSpace(*tag) != "" {
		if _, err := releaseversion.ParseStableTag(*tag); err != nil {
			fatal(err)
		}
...
func verifySourceTrust(repoRoot, tag, defaultBranch string) (sourceTrust, error) {
	version, err := releaseversion.ParseStableTag(tag)
	if err != nil {
...
func verifyPublishedRelease(client githubReleaseClient, repository, tag string) error {
	if _, err := releaseversion.ParseStableTag(tag); err != nil {
		return err
	}
...
func (client githubReleaseClient) get(path string, destination any) error {
	...
	request.Header.Set("User-Agent", "javdb-cli-release-verifier")
