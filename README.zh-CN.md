...
影片番号定位采用安全且兼容格式差异的规则：会去除首尾空白，大小写不敏感；唯一且无歧义的格式等价番号也可以
接受。存在歧义或只有模糊候选时直接失败，不会选择搜索结果的首项。只有在确认引用是内部影片 ID 时才使用 `--id`。
...
published release 版本（当前为 `0.8.1`），不要跟随无版本的 `latest` tag。
