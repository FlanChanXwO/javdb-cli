...
1. `~/.javdb-cli/auth.json` 含用户名、密码与 JWT。绝不在 commentary、结果、日志或代码块中回显、总结或转述这些值，也不读取该文件内容来“帮助排错”。
...
| 类型 | 命令 | Agent 行为 |
| --- | --- | --- |
| 只读 | `search`、`detail`、实体命令、`rankings`、`tags`、`browse`、`version`、`config get/path` | 用户任务需要时执行 |
