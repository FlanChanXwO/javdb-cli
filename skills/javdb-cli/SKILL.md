...
7. 影片媒体资产走两步:`javdb assets list NUMBER` 先看资产(编号只是位置),再用 selector 或 `--type` 过滤后管道到 `javdb assets download`。`-d DIR` 自动命名(`image-001.jpg`/`video-001.mp4`),`-o PATH` 只接受恰好一个资产。两者都是本地资源写入,目标不能已存在,不表示支持完整影片或磁力下载。`assets download` stdout 每行只输出最终路径,无装饰文本。
