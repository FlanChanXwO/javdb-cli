...
7. 影片媒体资产走两步:`javdb assets list NUMBER` 先看资产(编号只是位置),再用 selector 或 `--type` 过滤后管道到 `javdb assets download`。`-d DIR` 自动命名(`image-001.jpg`/`video-001.mp4`),`-o PATH` 只接受恰好一个资产。两者都是本地资源写入,目标不能已存在,不表示支持完整影片或磁力下载。
...
9. 影片媒体资产:`javdb assets list NUMBER` 输出资产序列(TTY 带编号描述;管道为 `TYPE<TAB>URL`),`javdb assets download` 消费该记录流并验证落盘。不要求自己解析 preview_images JSON、手写 Referer、下载 HLS segment 或调用 ffmpeg。该命令域只写入本地 thumbnail/preview 资源,不支持完整影片或磁力目标。失败时如实报告,不能把已包装的图片字节或不完整视频当作成功结果。
