---
name: javdb-cli-code-commenting
description: In javdb-cli, write and review intent-focused code comments, API documentation, docstrings, and numbered workflow stages. Use when an implementation changes a contract or non-obvious invariant, a multi-stage function needs navigation, or existing comments are noisy or stale. Keep natural-language choice neutral and preserve compiler/tool directives. Do not require comments for every function or statement.
---

# javdb-cli Code Commenting
...
Read root `AGENTS.md` and the affected owner before editing. This checked-in skill is self-contained; it does not require the personal `code-commenting` skill. Keep SDK/CLI ownership, pipeline versus asset-stream semantics, exact identity before writes, and media publication outcomes precise. A checksum or file write must not be described as authenticated or atomic without the actual guarantee; distinguish pre-commit failure from committed output. Use [javdb-cli-test](../javdb-cli-test/SKILL.md) for verification scope and [javdb-cli-develop](../javdb-cli-develop/SKILL.md) for any authorized structural change.
