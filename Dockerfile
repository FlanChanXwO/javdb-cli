...
# 固定基础镜像；不使用可变 tag。运行时 apt 包版本仍由 Debian 仓库解析。
...
# 预创建状态目录并固定属主与 0700 私有权限；首次挂载空命名 volume 时 Docker 会继承该 ownership。
RUN install -d -o javdb -g javdb -m 0700 /home/javdb/.javdb-cli
...
# /work 是默认工作目录，用于下载产物等 bind mount；目录需在切换到非 root 用户前
# 以 javdb 可写的属主和权限创建，保证默认容器可直接写入下载产物。
RUN install -d -o javdb -g javdb -m 0755 /work
WORKDIR /work
