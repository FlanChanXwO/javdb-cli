...
func TestSearchRejectsInvalidTypeBeforeRequest(t *testing.T) {
	requests := 0
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		requests++
		writer.Header().Set("Content-Type", "application/json")
	...
	streams := invocation.NewStreams(strings.NewReader(""), &bytes.Buffer{}, &bytes.Buffer{})
	cmd := New(&invocation.RootOptions{Host: server.URL}, streams)
	cmd.SetArgs([]string{"keyword", "--type", "invalid"})
	err := cmd.Execute()
	if err == nil || !strings.Contains(err.Error(), "type must be one of movie|code|series|actor|maker|director|list") {
		t.Fatalf("invalid type error = %v", err)
	}
	if requests != 0 {
		t.Fatalf("invalid type sent %d request(s), want none", requests)
	}
}
