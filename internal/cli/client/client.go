...
// token 的公开 SDK client。固定 host 完全绕过 route cache 与 selector。基线配置在 host 语义
// 校验与 SDK/transport 的 proxy 校验之后创建，无效 host 或 proxy 不落盘。
...
// resolveClient 解析运行时（校验 host、补齐 device UUID）并确定 baseURL：auto 走 route cache
// + 公开 SDK selector，固定 host 直接使用 runtime 的 BaseURL。基线配置由调用方在 buildClient
// （transport 校验 proxy）成功后创建。
