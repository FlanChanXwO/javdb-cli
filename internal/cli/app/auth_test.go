...
// auth store never leaks across tests or into a real user profile. paths.Dir
...
func TestWithOptionalAuthClientNoTokenAnonymous(t *testing.T) {
	isolateHome(t)
	aio := &IO{In: strings.NewReader(""), Out: &bytes.Buffer{}, Err: &bytes.Buffer{}}

	calls := 0
	err := WithOptionalAuthClient(&Flags{}, aio, func(c *javdb.Client) error {
		calls++
		if tok := c.Token(); tok != "" {
...
func TestWithOptionalAuthClientFallbackAnonymous(t *testing.T) {
	...
	aio := &IO{In: strings.NewReader(""), Out: &bytes.Buffer{}, Err: &errb}
