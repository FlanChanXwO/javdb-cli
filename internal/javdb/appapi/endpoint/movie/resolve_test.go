...
func TestMovieEndpointResolveMovieIDUsesSafeSearch(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:      "accepts formatting-equivalent input",
			input:     "  ssis589  ",
			wantID:    "id-exact",
			wantQuery: "ssis589",
			movies: []map[string]any{
...
func TestResolveNumber(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:  "unique formatting-equivalent candidate",
			input: "  ssis589  ",
			movies: []map[string]any{
				{"number": "SSIS-589", "id": "id-a"},
			},
		...
		{
			name:  "formatting-equivalent candidates with different ids are ambiguous",
			input: "SSIS 589",
			movies: []map[string]any{
				{"number": "SSIS-589", "id": "id-a"},
				{"number": "SSIS589", "id": "id-b"},
			},
			wantErr: true,
		},
		{
			name:  "fuzzy-only candidate is rejected",
			input: "SSIS-589",
			movies: []map[string]any{
				{"number": "SSIS-58X", "id": "id-near"},
			},
		...
		t.Run(tc.name, func(t *testing.T) {
			id, err := ResolveNumber(tc.movies, tc.input)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("ResolveNumber returned %q, want error", id)
				}
				return
			}
			if err != nil {
