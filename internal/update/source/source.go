...
const (
	InstallSourceDevelopment = model.InstallSourceDevelopment
	InstallSourceHomebrew    = model.InstallSourceHomebrew
	InstallSourceGoInstall   = model.InstallSourceGoInstall
	InstallSourceRelease     = model.InstallSourceRelease
)
...
func detectHomebrewSource(executable string, deps sourceDetector) (InstallSource, bool, error) {
	formula, kegRoot, ok := homebrewKeg(executable, process.ExecutableName(deps.goos))
	if !ok {
		return "", false, nil
	}
...
func isGoInstall(executable string, deps sourceDetector) (bool, error) {
	...
	expected, err := deps.evalSymlinks(filepath.Join(binDirectory, process.ExecutableName(deps.goos)))
	if err != nil {
